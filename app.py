...
def main():
    # Create logger
    logging.basicConfig(filename='/home/pi/data/growlab.log', level=logging.INFO, format='%(asctime)s %(message)s')
    ...
    while True:
        ...
        if (config["time"]["start"] <= hour <= config["time"]["end"]):
            ...
            spec.save_html("{}/image.jpg".format(pwd), config["preview"]["directory"], readings)
